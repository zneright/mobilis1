...
import { collection, query, where, getDocs, doc, onSnapshot, setDoc, addDoc } from 'firebase/firestore';
import {
    Keypair,
    Networks,
    TransactionBuilder,
...
const Dashboard: React.FC = () => {
    ...
    const handleSettleLoan = async () => {
        ...
        try {
            ...
            // Fetch Cooperative Admin Public Key (Matches the driver's todaAffiliation)
            ...
            // ==========================================
            // 2. CALCULATE FEES (0.5% Split)
            // ==========================================

            // Calculate the 0.5% fee based on the current debt
            const totalFee = debtState * 0.005;

            // Split the fee 50/50 between the Coop and the Superadmin
            // Stellar requires amounts to be formatted as strings with up to 7 decimal places
            const splitFeeAmount = (totalFee / 2).toFixed(7).toString();
            ...
            let txBuilder = new TransactionBuilder(account, { fee: "10000", networkPassphrase: NETWORK_PASSPHRASE })
                // Operation A: Settle the loan state in the Soroban Contract
.addOperation(contract.call("settle_loan", 
    nativeToScVal(activePubKey, { type: 'address' }) // <-- Removed the [ ] brackets
))
                // Operation B: Route fee split to the Cooperative
                .addOperation(Operation.payment({
                    destination: coopPubKey,
                    asset: Asset.native(),
                    amount: splitFeeAmount
                }))
                // Operation C: Route fee split to the Superadmin
                .addOperation(Operation.payment({
            ...
            if (txResult.status === "SUCCESS") {
                // Log the complex transaction to Firebase
                await addDoc(collection(db, 'transactions'), {
                    txHash: response.hash,
                    senderUid: stellarData?.uid,
                    senderName: (stellarData as any)?.fullName || 'Node Operator',
                ...
                throw new Error(`Execution failed on ledger: ${txResult.status}`);
            }
