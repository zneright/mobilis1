...
import { Coins, RefreshCw, Eye, EyeOff, Copy, AlertTriangle, ArrowDownLeft, ArrowUpRight, Banknote, ExternalLink, X, Link as LinkIcon, Unlink } from 'lucide-react';
...
const Dashboard: React.FC = () => {
    ...
    // --- NEW: EXTERNAL WALLET STATE ---
    const [externalWallet, setExternalWallet] = useState<string | null>(null);
    const activePubKey = externalWallet || stellarData?.publicKey; // Dynamic active address
        ...
        const fetchLedgerData = async () => {
            ...
            try {
                ...
                if (data.balances) {
                    ...
                    setXlmBalance('0.00'); // Account not funded yet
                }
            ...
            if (stellarData?.role === 'driver') {
                try {
                    ...
                    // We can use any account as the 'source' for a pure read simulation
    ...
    const handleConnectWallet = async () => {
        if (await isInstalled()) {
            ...
            alert("Freighter is not installed. Please install the extension or open this app inside a compatible Web3 mobile browser.");
        }
    };
    ...
    const signAndSubmitTx = async (server: rpc.Server, preparedTx: any) => {
        if (externalWallet) {
            ...
            // Sign silently with internal Firebase key
    ...
    const handleSendXLM = async (e: React.FormEvent) => {
        ...
        try {
            const server = new rpc.Server(RPC_SERVER);
            // Fetch sequence number for the active account
                        ...
                        <div className="w-full max-w-4xl mx-auto flex flex-col gap-4">
                            <div className="flex justify-between items-end mb-2">
                                ...
                                <div className="flex gap-2">
                                    {externalWallet ? (
                                        <button onClick={handleDisconnectWallet} className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold font-mono bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400 border border-red-200 dark:border-red-500/20 shadow-sm transition-colors hover:bg-red-100 dark:hover:bg-red-500/20">
                                            <Unlink className="w-3.5 h-3.5" /> Disconnect
                                        </button>
                                    ) : (
                                        <button onClick={handleConnectWallet} className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold font-mono bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 shadow-sm transition-colors hover:bg-blue-100 dark:hover:bg-blue-500/20">
                                            <LinkIcon className="w-3.5 h-3.5" /> Connect Wallet
                                        </button>
                                    )}
                                    <button onClick={() => setCurrencyMode(p => p === 'PHP' ? 'XLM' : 'PHP')} className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold font-mono bg-white dark:bg-[#0a0a14] border border-gray-200 dark:border-white/10 shadow-sm">
                                ...
                                <div className="col-span-1 md:col-span-3 bg-white dark:bg-[#0a0a14] border border-gray-200 dark:border-white/10 rounded-[2rem] p-6 sm:p-8 shadow-xl">
                                    {externalWallet ? (
                                        <div className="flex flex-col items-center justify-center py-6 text-center">
                                            ...
                                            <h4 className="text-lg font-bold mb-2">External Wallet Connected</h4>
                                            <p className="text-gray-500 text-sm mb-4">You are currently managing your assets via Freighter or a Web3 compatible browser.</p>
