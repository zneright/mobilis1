...
const Dashboard: React.FC = () => {
    ...
    const handleSettleLoan = async () => {
        ...
        try {
            ...
            console.log("Building Unified Settlement Transaction...");

            // 3. Create ONE transaction that does all three actions at once
            let account = await server.getAccount(activePubKey);

            // We increase the fee slightly to account for the multiple operations in one TX
            let settlementTxBuilder = new TransactionBuilder(account, { fee: "20000", networkPassphrase: NETWORK_PASSPHRASE })
                // Action A: Pay Cooperative
                .addOperation(Operation.payment({
                    destination: coopPubKey,
                    asset: Asset.native(),
                    amount: totalToCoopAmount
                }))
                // Action B: Pay Superadmin
                .addOperation(Operation.payment({
                    destination: superadminPubKey,
                    asset: Asset.native(),
                    amount: superadminFeeAmount
                }))
                // Action C: Clear Debt on Smart Contract Ledger
            ...
            console.log("Simulating, Preparing, and Signing unified transaction...");

            // Smart contract calls MUST be simulated/prepared before signing
            const preparedSettlementTx = await server.prepareTransaction(settlementTxBuilder);
            const settlementResponse = await signAndSubmitTx(server, preparedSettlementTx);

            if (settlementResponse.status === "ERROR") {
                console.error("Submission failed details:", settlementResponse.errorResult);
                throw new Error(`Payment failed. Check console for details.`);
            }

            // 4. Wait for the network to confirm the block
            let txResult = await server.getTransaction(settlementResponse.hash);
            while (txResult.status === "NOT_FOUND" || txResult.status === "PENDING") {
                await new Promise(resolve => setTimeout(resolve, 2000));
                txResult = await server.getTransaction(settlementResponse.hash);
            }

            if (txResult.status !== "SUCCESS") {
                throw new Error(`Transaction reverted on ledger: ${txResult.status}`);
            }
            ...
            await addDoc(collection(db, 'transactions'), {
                txHash: settlementResponse.hash,
                senderUid: stellarData?.uid,
                senderName: (stellarData as any)?.fullName || 'Node Operator',
            ...
            alert(`Settlement Complete! Principal and fees routed, and ledger cleared successfully.`);
            setTimeout(() => fetchLedgerData(), 3000);
