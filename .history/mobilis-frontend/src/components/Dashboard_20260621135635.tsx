...
const Dashboard: React.FC = () => {
        ...
        try {
            const unsub = onSnapshot(doc(db, 'settings', 'global'), (docSnap) => {
                if (docSnap.exists()) {
                    ...
                    setDoc(doc(db, 'settings', 'global'), { activeNetwork: 'TESTNET' }).catch((e) => console.error("[Dashboard -> Network Sync Setup Error]:", e));
                }
            }, (error) => {
                console.error("[Dashboard -> Network Snapshot Listener Error]:", error);
            });
            return () => unsub();
        } catch (e) {
            console.error("[Dashboard -> Network Initialization Error]:", e);
        }
    }, []);
    ...
    const handleNetworkChange = async (newNetwork: 'TESTNET' | 'PUBLIC') => {
        try {
            ...
            console.error("[Dashboard -> handleNetworkChange Error]:", error);
        }
    };
    ...
    const handleFullSignOut = async () => {
        try {
            // 1. Wipe Web3 external wallet memory
            setExternalWallet(null);
            localStorage.removeItem('externalWalletConnected');

            // 2. Kill Firebase Auth session (Destroys internal 'Created Wallet' memory)
            await signOut(auth);

            // 3. Hard reload to flush all React context and cached variables
    ...
    const handleDisconnectWallet = () => {
        setExternalWallet(null);
        localStorage.removeItem('externalWalletConnected');
        // Note: This only disconnects external Web3. It falls back to Firebase internal wallet.
        // If the user wants to remove the internal Firebase wallet, they use the Header Sign Out.
    };
    ...
    const handleRequestAdvance = async (amount: number) => {
        ...
        try {
            ...
            // FETCH COOPERATIVE WALLET (Source of Funds)
            if (stellarData?.role === 'driver') {
            ...
            fetchLedgerData(); // Sync updated balances
        } catch (e: any) {
    ...
    const handleSettleLoan = async () => {
        ...
        try {
            ...
            // Log the settlement to Firebase
            await addDoc(collection(db, 'transactions'), {
            ...
            alert(`Transaction Failed: ${e.message || "Ensure you have sufficient balance to cover the debt and gas fees."}`);
        } finally {
            setIsProcessing(false);
        }
    };
    ...
    return (
        <div className="flex h-screen bg-gray-50 dark:bg-[#060610] text-gray-900 dark:text-white font-sans overflow-hidden">
            ...
            <div className="flex-1 flex flex-col h-full overflow-y-auto relative">
                {/* Applied handleFullSignOut to the Header component here */}
        ...
        <div className="flex h-screen bg-gray-50 dark:bg-[#060610] text-gray-900 dark:text-white font-sans overflow-hidden">
            ...
            <div className="flex-1 flex flex-col h-full overflow-y-auto relative">
                ...
                <main className="flex-1 w-full max-w-6xl mx-auto p-4 sm:p-8 flex flex-col items-center pb-28 md:pb-8">
                    ...
                    {activeTab === 'vault' && <VaultTab stellarData={stellarData} externalWallet={externalWallet} activePubKey={activePubKey} xlmBalance={xlmBalance} assetBalances={assetBalances} currencyMode={currencyMode} setCurrencyMode={setCurrencyMode} formatCurrency={formatCurrency} setShowWalletModal={setShowWalletModal} handleDisconnectWallet={handleDisconnectWallet} setShowReceiveModal={setShowReceiveModal} setShowSendModal
