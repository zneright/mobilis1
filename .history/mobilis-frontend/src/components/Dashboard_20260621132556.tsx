...
const Dashboard: React.FC = () => {
    ...
    const executeContractCall = async (functionName: string, args: any[]) => {
        ...
        try {
            ...
            // Replace the multi-operation builder with this single line inside handleSettleLoan:
    ...
    const handleSettleLoan = async () => {
        ...
        try {
            // Your deployed contract already calculates the 0.2% and 0.3% fees internally.
            // We just need to tell it to execute the settlement.
            const server = new rpc.Server(RPC_SERVER);
            const account = await server.getAccount(activePubKey);
            const contract = new Contract(CONTRACT_ID);
