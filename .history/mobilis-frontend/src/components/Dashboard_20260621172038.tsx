...
const Dashboard: React.FC = () => {
    ...
    const handleSettleLoan = async () => {
        ...
        try {
            ...
            // 3. TRANSACTION A: Send the XLM back to the Admin
            let account = await server.getAccount(activePubKey);
            let coopPaymentTxBuilder = new TransactionBuilder(account, { fee: "1000", networkPassphrase: NETWORK_PASSPHRASE })
                .addOperation(Operation.payment({ destination: coopPubKey, asset: Asset.native(), amount: totalToCoopAmount }))
                .setTimeout(30).build();

            const preparedCoopTx = await server.prepareTransaction(coopPaymentTxBuilder);
            const coopResponse = await signAndSubmitTx(server, preparedCoopTx);

            if (coopResponse.status === "ERROR") throw new Error(`Coop Payment failed.`);

            let coopTxResult = await server.getTransaction(coopResponse.hash);
            while (coopTxResult.status === "NOT_FOUND" || coopTxResult.status === "PENDING") {
                await new Promise(resolve => setTimeout(resolve, 2000));
                coopTxResult = await server.getTransaction(coopResponse.hash);
            }

            console.log("Routing Platform Fee...");

            // 4. TRANSACTION B: Send the 0.2% to the Superadmin
            // We MUST fetch the account again to get the updated sequence number
            account = await server.getAccount(activePubKey);
            let superAdminTxBuilder = new TransactionBuilder(account, { fee: "1000", networkPassphrase: NETWORK_PASSPHRASE })
                .addOperation(Operation.payment({ destination: superadminPubKey, asset: Asset.native(), amount: superadminFeeAmount }))
                .setTimeout(30).build();

            const preparedSuperAdminTx = await server.prepareTransaction(superAdminTxBuilder);
            const superAdminResponse = await signAndSubmitTx(server, preparedSuperAdminTx);

            if (superAdminResponse.status === "ERROR") throw new Error(`Superadmin Payment failed.`);

            let saTxResult = await server.getTransaction(superAdminResponse.hash);
            while (saTxResult.status === "NOT_FOUND" || saTxResult.status === "PENDING") {
                await new Promise(resolve => setTimeout(resolve, 2000));
                saTxResult = await server.getTransaction(superAdminResponse.hash);
            }

            console.log("Clearing Smart Contract Debt Ledger...");

            // 5. Update the Smart Contract to wipe the debt
            ...
            // 6. Update Firebase History
            await addDoc(collection(db, 'transactions'), {
                txHash: coopResponse.hash, // Using the main Coop payment hash for the receipt
                senderUid: stellarData?.uid,
                senderName: (stellarData as any)?.fullName || 'Node Operator',
