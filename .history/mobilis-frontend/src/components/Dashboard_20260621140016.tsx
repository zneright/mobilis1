...
const Dashboard: React.FC = () => {
    ...
    const handleInjectLiquidity = async (amount: number) => {
        ...
        try {
            ...
            // ⚠️ FIX: Soroban Contracts (C...) cannot receive classic Stellar payments.
            // We must invoke the "transfer" function on the official Native XLM Contract.
            const NATIVE_CONTRACT_ID = appNetwork === 'TESTNET'
                ? 'CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC'
                : 'CAS3J7YVGRSYCEEXCGFSZOOZ7PH45STDTQQUYQ2G3HQR67QDBR3Z23C5';
            ...
            let tx = new TransactionBuilder(account, { fee: "10000", networkPassphrase: NETWORK_PASSPHRASE })
                .addOperation(nativeContract.call(
                    "transfer",
                    nativeToScVal(activePubKey, { type: 'address' }), // From Admin Wallet
                    nativeToScVal(CONTRACT_ID, { type: 'address' }),  // To Mobilis Contract
                    nativeToScVal(Math.floor(amount * 10000000).toString(), { type: 'i128' }) // Amount in stroops
                ))
                .setTimeout(30).build();
