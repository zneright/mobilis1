...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetchLedgerData = async () => {
            ...
            try {
                // Fetch Balance (Gracefully handle 404s for unfunded accounts)
            ...
            if (stellarData?.role === 'driver') {
                try {
                    ...
                    // Contract debt query failed or account doesn't exist
                    setDebtState(0);
                }
            }
        };
        ...
        if (connection.isConnected) {
            try {
                ...
                if (accessResult.error) {
                    alert(`Connection failed: ${accessResult.error}`);
                } else if (accessResult.address) {
                    setExternalWallet(accessResult.address); // <-- Extract the address property
                }
            } catch (e) {
    ...
    const signAndSubmitTx = async (server: rpc.Server, preparedTx: any) => {
        if (externalWallet) {
            // Destructure the new return object
            ...
            // Pass the extracted string to the TransactionBuilder
