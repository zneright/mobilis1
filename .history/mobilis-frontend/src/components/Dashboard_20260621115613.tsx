...
import {
    ...
    requestAccess,
    signTransaction,
    isConnected // Fixed: Imported missing function
...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        const checkAutoConnect = async () => {
            // 1. Check if the wallet extension exists in the browser
            if (await isConnected()) {
                // 2. Check if the user has already granted permission to this app
                if (await isAllowed()) {
                    try {
    ...
    const handleConnectWallet = async () => {
        if (await isConnected()) { // Changed from isInstalled()
            try {
                const pubKey = await requestAccess();
                if (typeof pubKey === 'string') {
                    setExternalWallet(pubKey);
                } else if (pubKey && (pubKey as any).address) {
                    setExternalWallet((pubKey as any).address); // Fallback for older Freighter return objects
                }
            } catch (e) {
                alert("Connection request was rejected or the wallet is locked.");
            }
        } else {
            const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
            if (isMobile) {
                alert("To connect an external wallet on mobile, please open this website directly inside the Lobstr app's 'Discover' browser, or use a Web3 compatible mobile browser.");
            } else {
                alert("Freighter is not installed. Please install the Freighter extension for your desktop browser.");
            }
        }
    };
                ...
                <main className="flex-1 w-full max-w-6xl mx-auto p-4 sm:p-8 flex flex-col items-center pb-32 md:pb-8">
                        ...
                        <div className="w-full max-w-4xl mx-auto flex flex-col gap-4">
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end mb-2 gap-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {/* Superadmins can connect external wallets too now! */}
                                    {externalWallet ? (
                                        ...
                                        <button onClick={handleConnectWallet} className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold font-mono bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 shadow-sm transition-colors hover:bg-blue-100 dark:hover:bg-blue-500/20">
                                ...
                                <div className="col-span-1 md:col-span-3 bg-white dark:bg-[#0a0a14] border border-gray-200 dark:border-white/10 rounded-[2rem] p-6 sm:p-8 shadow-xl">
                                    {externalWallet ? (
                                        <div className="flex flex-col items-center justify-center py-6 text-center">
                                            ...
                                            <p className="text-gray-500 text-sm mb-4">You are currently managing your assets via an external application.</p>
                                    ...
                                    txHistory.map((tx: any, idx: number) => (
                                        <div key={idx} className="p-5 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/5 flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                                            <div className="flex-1">
                                                ...
                                                {/* Added Sender and Memo displays */}
                    ...
                    {activeTab === 'profile' && (
                        <div className="w-full max-w-xl mx-auto bg-white dark:bg-[#0a0a14] border border-gray-200 dark:border-white/10 rounded-[2rem] p-6 sm:p-8 shadow-xl">
                            ...
                            {/* Added User Identity & Cooperative Info Read-Only Cards */}
