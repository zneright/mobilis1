...
const Dashboard: React.FC = () => {
    ...
    // Add this new state right below your other data states
    ...
    const fetchLedgerData = async () => {
        ...
        try {
            // 1. Fetch Personal Wallet Balance (Driver's wallet, or Admin's fee wallet)
            ...
            // 2. NEW: If user is an Admin, fetch the Smart Contract Treasury Balance!
        ...
        // ... (Keep your existing if (stellarData?.role === 'driver') logic here!)
        ...
        // --- DRIVER: SECURE ON-CHAIN ADVANCE ---
        const handleRequestAdvance = async (amount: number) => {
            ...
            try {
                ...
                console.log("Executing Smart Contract: Pulling physical XLM from Treasury...");

                // The Smart Contract now handles the physical transfer from Treasury -> Driver
                ...
                // Log to Firestore for the History Tab
                await addDoc(collection(db, 'transactions'), {
                ...
                alert(`Success! ${borrowVal} XLM advance has been approved and deposited into your wallet by the Smart Contract.`);
                setTimeout(() => fetchLedgerData(), 3000);

            } catch (e: any) {
                console.error("Smart Contract Advance Failed:", e);
                alert(`Advance Failed: ${e.message || "Ensure the Cooperative Treasury has sufficient liquidity."}`);
            } finally {
                setIsProcessing(false);
            }
        };

        // --- DRIVER: SECURE ON-CHAIN SETTLEMENT ---
        const handleSettleLoan = async () => {
            if (!activePubKey || debtState <= 0) return;
            setIsProcessing(true);

            try {
                console.log("Executing Smart Contract: Routing Principal & Fees...");

                const totalFee = debtState * 0.005; // 0.5% total fee just for UI logging

                // The Smart Contract automatically pulls the Principal + 0.3% to itself, 
                // and sends the 0.2% directly to the Superadmin Platform Wallet.
                ...
                console.error("Smart Contract Settlement Failed:", e);
                alert(`Transaction Failed: ${e.message || "Ensure you have sufficient balance to cover the principal and the 0.5% routing fees."}`);
            } finally {
                setIsProcessing(false);
            }
        };
