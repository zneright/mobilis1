...
const Dashboard: React.FC = () => {
    ...
    const handleSettleLoan = async () => {
        ...
        try {
            ...
            const preparedFeeTx = await server.prepareTransaction(feeTxBuilder.build());
            const feeResponse = await signAndSubmitTx(server, preparedFeeTx);
            ...
            // CRITICAL: Fetch account again to get the incremented sequence number!
            ...
            // ==========================================
            await addDoc(collection(db, 'transactions'), {
                txHash: contractResponse.hash, // Storing the primary contract hash
                feeTxHash: feeResponse.hash,   // Storing the secondary fee hash for records
                senderUid: stellarData?.uid,
                senderName: (stellarData as any)?.fullName || 'Node Operator',
