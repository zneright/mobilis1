...
const Dashboard: React.FC = () => {
    ...
    const handleSettleLoan = async () => {
        ...
        try {
            ...
            // 3. Physically send the XLM back to the Admin and Superadmin
            ...
            const preparedPaymentTx = await server.prepareTransaction(paymentTxBuilder);
            const paymentResponse = await signAndSubmitTx(server, preparedPaymentTx);

            if (paymentResponse.status === "ERROR") throw new Error(`Payment failed.`);
